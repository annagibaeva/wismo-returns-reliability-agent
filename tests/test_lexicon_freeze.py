...
def test_a_first_freeze_needs_no_force(fake_repo: Path) -> None:
    ...
    assert sorted(p.name for p in freeze.FREEZE_DIR.iterdir()) == [
        ...
        "lexicons.json", "llm.json", "route.json"]
...
def _agent_modules() -> set[str]:
    """Every .py file under agent/, recursive, as "agent/<relative path>" posix paths.

    Recursive because a nested package (e.g. agent/_probe_sub/cache.py) is exactly as
    capable of hiding a lexicon as a flat one -- a non-recursive glob would let a module
    one directory down sit unscanned *and* unexcused, invisible to both MODULES and
    NON_LEXICON_MODULES, which is the gap this whole test exists to close.

    Excludes any path with a `__pycache__` or dot-prefixed component (`.mypy_cache`,
    `.pytest_cache`, ...) -- tool-generated bytecode/cache directories that are never
    source, so they cannot hold a lexicon a human wrote. Nothing else is excluded:
    `__init__.py` files count, because an innocuous-looking re-export module is exactly
    the kind of place a keyword list could be added without anyone treating it as
    "real" code.
    """
    agent_root = ROOT / "agent"
    return {
        f"agent/{p.relative_to(agent_root).as_posix()}"
        for p in agent_root.rglob("*.py")
        if not any(part == "__pycache__" or part.startswith(".") for part in p.relative_to(agent_root).parts)
    }


# The complete set of .py files under agent/ as of this task, transcribed by hand (not
# derived from MODULES, NON_LEXICON_MODULES, or _agent_modules() itself) so it stands as
# an independent witness that the enumeration actually looked at something real. Without
# this, a wrong ROOT, a renamed/moved agent/, or a broken glob makes `found` empty --
# `unaccounted` becomes empty right along with it, and the test passes while scanning
# nothing, which is the exact "check passing while guarding nothing" failure this
# mechanism exists to rule out. Checked with `<=` (subset), not `==`, so a *new*
# legitimate module never needs an edit here -- only removing or renaming one of these
# six foundational files would.
_AGENT_MODULES_AT_TASK_TIME = frozenset({
    "agent/__init__.py",
    "agent/agent.py",
    "agent/extract.py",
    "agent/lexicons.py",
...
def test_every_agent_module_is_scanned_or_explicitly_excused() -> None:
    ...
    moment it is created, e.g. a future agent/cache.py or agent/_probe_sub/cache.py --
    before anyone has to notice on their own.
    ...
    assert found >= _AGENT_MODULES_AT_TASK_TIME, (
        "the agent/ module scan found fewer files than expected, so it is not actually "
        f"scanning the real agent/ directory: missing {sorted(_AGENT_MODULES_AT_TASK_TIME - found)}. "
        "Check ROOT and _agent_modules() in tests/test_lexicon_freeze.py."
    )
