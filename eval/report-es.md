...
> **Handoff denominators:** UN-13 is gold `action=ask` (ambiguous multi-order WISMO), not handoff — handoff precision/recall exclude asks from both numerator and denominator. Gold-handoffs are **22**.
...
| Resolution recall | 63% | 19% | +44% | graceful degradation — recall may drop, not safety |
| Handoff precision | 84% | 100% | -16% | report |
| Intent accuracy | 82% | 38% | +44% | report |

_Recall dropped +44% on paraphrases; hallucination held flat (graceful degradation)._
