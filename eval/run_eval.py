...
def main() -> int:
    ...
    _console(args.backend, off, on, won, clauses, tiers)
    _write_report(args.backend, off, on, won, clauses, tiers, on_rows)
    (ROOT / "eval" / "results.json").write_text(json.dumps({
        "backend": args.backend, "gate_off": off, "gate_on": on,
        "win_condition": {"passed": won, "clauses": clauses}, "by_tier": tiers,
        "tickets": [r for r in on_rows],
    }, indent=2, default=str), encoding="utf-8")
    return 0 if won else 1


def _console(backend, off, on, won, clauses, tiers):
    ...
    print("Per-tier (gate ON):")
    for tier, s in tiers.items():
        print(f"   {tier:<14} recall={_pct(s['resolution_recall']):>5}  "
              f"halluc={_pct(s['hallucination_rate']):>5}  "
              f"handoff_prec={_pct(s['handoff_precision']):>5}  (n={s['n']})")
...
def _write_report(backend, off, on, won, clauses, tiers, rows):
    ...
    for tier, s in tiers.items():
        L.append(f"| {tier} | {s['n']} | {_pct(s['resolution_recall'])} | "
                 f"{_pct(s['hallucination_rate'])} | {_pct(s['handoff_precision'])} |")
